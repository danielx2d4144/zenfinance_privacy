...
import { ASSET_ID, type WithdrawCollateralIntent } from "../schemas.js";
...
export async function handleWithdrawCollateral(
  pool: Pool,
  intent: IntentRow,
  body: z.infer<typeof WithdrawCollateralIntent>,
): Promise<void> {
  const { shieldedPositionPool } = getChainClients();
  await verifyAndCall({
    pool,
    intent,
    circuit: "withdraw_collateral",
    ...
    targetFunction: "withdrawCollateral",
    targetArgs: [
      ASSET_ID[body.asset],
