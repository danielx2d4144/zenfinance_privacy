...
- [ ] Relayer keeper implemented (consumes the aggregation receipt; calls `IVerifyProofAggregation` on Base Sepolia at `0x312468Eb...` then forwards to Horizen)
- [ ] Full chain test (Kurier → zkVerify → Base Sepolia proxy → Horizen contract) succeeds
- [ ] Day-8 tests run: T-8.1 ___, T-8.2 ___, T-8.3 ___, T-8.4 ___
